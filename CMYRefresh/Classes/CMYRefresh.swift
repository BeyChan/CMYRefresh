...
extension UIScrollView {
    @discardableResult public func addCMYRefresh(position: CMYRefreshPosition, config: CMYRefreshConfig? = nil, refreshView: CMYRefreshViewProtocol? = nil, action: ((CMYRefresh) -> Void)?) -> CMYRefresh {
        let refresh = CMYRefresh(scrollView: self, position: position, config: config, refreshView: refreshView, action: action)
        addSubview(refresh)
        switch position {
        case .top:
            self.cmy_topRefresh = refresh
        case .left:
            self.cmy_leftRefresh = refresh
        case .bottom:
            self.cmy_bottomRefresh = refresh
        case .right:
            self.cmy_rightRefresh = refresh
        }
        return refresh
    }
    
    public var cmy_topRefresh: CMYRefresh? {
        get {
            return objc_getAssociatedObject(self, &kTopRefreash) as? CMYRefresh
        }
        set {
            objc_setAssociatedObject(self, &kTopRefreash, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
    public var cmy_leftRefresh: CMYRefresh? {
        get {
            return objc_getAssociatedObject(self, &kLeftRefreash) as? CMYRefresh
        }
        set {
            objc_setAssociatedObject(self, &kLeftRefreash, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
    public var cmy_bottomRefresh: CMYRefresh? {
        get {
            return objc_getAssociatedObject(self, &kBottomRefreash) as? CMYRefresh
        }
        set {
            objc_setAssociatedObject(self, &kBottomRefreash, newValue, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        }
    }
    public var cmy_rightRefresh: CMYRefresh? {
        get {
            return objc_getAssociatedObject(self, &kRightRefreash) as? CMYRefresh
        }
        set {
...
open class CMYRefresh: UIView {
    ...
    open var position: CMYRefreshPosition
    open var config: CMYRefreshConfig
    open var state: CMYRefreshState = .idle {
        willSet {
    ...
    // MARK: - Initialization
    public init(scrollView: UIScrollView, position: CMYRefreshPosition, config: CMYRefreshConfig? = nil, refreshView: CMYRefreshViewProtocol? = nil, action: ((CMYRefresh) -> Void)?) {
        self.position = position
        self.config = config ?? CMYRefreshConfig()
        self.scrollView = scrollView
        self.refreshingCallback = action
        self.refreshView = refreshView ?? CMYRefreshView(position: position)
    ...
    var refreshView: CMYRefreshViewProtocol!
    var refreshingCallback: ((CMYRefresh) -> Void)? = nil
